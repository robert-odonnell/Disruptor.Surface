...
/// </para>
/// </summary>
public sealed class SurfaceQuery<T>
    where T : class, IEntity, new()
{
    ...
    public SurfaceQuery(string table) : this(table, filter: null, pinnedId: null, includes: [], orderClauses: [], limitCount: null, startAt: null) { }
    ...
    public SurfaceQuery<T> Where(IPredicate predicate)
        => new(
            Table,
            Filter is null ? predicate : Predicate.And(Filter, predicate),
            PinnedId,
            Includes,
            OrderClauses,
            LimitCount,
    ...
    /// methods on <see cref="SurfaceQuery{T}"/> are the ergonomic surface — this is the
    ...
    public SurfaceQuery<T> WithInclude(IIncludeNode node)
    {
        ...
        return new SurfaceQuery<T>(Table, Filter, PinnedId, next, OrderClauses, LimitCount, StartAt);
    }
    ...
    private SurfaceQuery<T> Append(OrderClause clause)
    {
        ...
        return new SurfaceQuery<T>(Table, Filter, PinnedId, Includes, next, LimitCount, StartAt);
    }
    ...
    /// </summary>
    public SurfaceQuery<T> Limit(int count)
    ...
    /// </summary>
    public SurfaceQuery<T> Start(int count)
    ...
    /// </summary>
    public Task<IReadOnlyList<T>> ExecuteAsync(SurrealClient db, CancellationToken ct = default)
    ...
    public Task<IReadOnlyList<T>> ExecuteAsync(SurrealTransaction tx, CancellationToken ct = default)
    ...
    /// <see cref="SurfaceQueryCompiler.CompileProjection"/> and materialises each row through the
    ...
    public (string Sql, SurrealObject Bindings) Compile()
        => SurfaceQueryCompiler.Compile(Table, Filter, PinnedId, Includes, OrderClauses, LimitCount, StartAt);
    ...
    /// without taking a dependency on the internal <see cref="SurfaceQueryCompiler"/>. Throws
    /// when <see cref="Includes"/> are present — id-only selection is flat by definition.
    /// </summary>
    public (string Sql, SurrealObject Bindings) CompileIdsOnly()
    {
        if (Includes.Count > 0)
        {
            throw new InvalidOperationException(
        ...
        return SurfaceQueryCompiler.CompileIdsOnly(Table, Filter, PinnedId, OrderClauses, LimitCount, StartAt);
    }
    ...
    public Task<IReadOnlyList<T>> ExecuteIntoSessionAsync(
        ...
        => ExecuteIntoSessionAsync(session, db.QueryAsync, ct);

    /// <inheritdoc cref="ExecuteIntoSessionAsync(SurrealSession, SurrealClient, CancellationToken)"/>
    public Task<IReadOnlyList<T>> ExecuteIntoSessionAsync(
        SurrealSession session,
        SurrealTransaction tx,
        CancellationToken ct = default)
        => ExecuteIntoSessionAsync(session, tx.QueryAsync, ct);
        ...
        foreach (var node in nodes)
        {
            switch (node)
            {
                ...
                case IncludeChildrenNode children:
                    ...
                    {
                        if (childRow is not SurrealObjectValue childObj)
                        {
                            continue;
                        }
                ...
                case IncludeRelationNode relation:
                    ...
                    if (relation.IdsOnly)
                    {
                        ...
                        {
                            if (edgeRow is not SurrealObjectValue edgeObj)
                            {
                                continue;
                            }
                    ...
                    else
                    {
                        ...
                        {
                            if (targetRow is not SurrealObjectValue targetObj)
                            {
                                continue;
                            }
