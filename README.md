...
**Requirements:** .NET 10. **License:** [MIT](LICENSE). **Package status:** published to NuGet as `Disruptor.Surface.Runtime` + `Disruptor.Surface.Generator` (`1.0.0`, see [quickstart](docs/quickstart.md)); building from a checkout also works (see [Building](#building)).

> **Status: 1.0.0 — first stable release.** Functional end-to-end against a live SurrealDB v3 (v3.1.4), built on the stable `Disruptor.Surreal` 1.0.0 transport. `1.0` is an API-stability line, not feature-completion — see [`docs/notes.md`](docs/notes.md) for the engineering log and [`Improvements.md`](Improvements.md) for the maintained backlog of post-1.0 additive work.
